...
const cats:Record<Cat,{name:string;short:string;desc:string;image:string;accent:string}>={
  priest:{name:"祭司媒合",short:"祭",desc:"找祈禱、補血與輔助，或刊登自己目前可配合狀態。",image:"/priest.jpg",accent:"#e9872d"},
...
function friendlyError(message:string){
  ...
  if(m.includes("留言太快"))return "留言太快了，請稍候 30 秒再留言";
...
const demo:Listing[]=order.map((c,i)=>({
  ...
  subtitle:["Lv.83 祭司｜現在可配合","Lv.21～30｜現在","今晚 21:30","晚上活躍","Lv.43 僧侶｜晚上玩家"][i],
  server:gameServers[i%gameServers.length],status:"active",
  description:[
    "可配合祈禱與補血，今晚可長時間配合。",
    "現在準備開，新手也可以，預計連打幾場。",
    "目前 4 / 6，缺 1 位祭司與 1 位打手。",
    "休閒為主，新手、回鍋都歡迎，不強制語音。",
    "喜歡解任務、聊天與慢慢玩，希望找長期夥伴。"
  ][i],
  tags:[["祈禱","補血"],["超綠","新手可"],["祭司","打手"],["新手友善","休閒"],["任務","聊天","長期固定"]][i],
...
export default function Page(){
  ...
  const[adminCommentReports,setAdminCommentReports]=useState<AdminCommentReport[]>([]);
  ...
  async function refreshAdminStatus(){
    ...
    if(active){
      ...
      void refreshAdminCommentReports();
    }else{
      setAdminReports([]);
      setAdminCommentReports([]);
    }
  ...
  async function refreshAdminCommentReports(){
    if(!supabase||!user)return;

    const{data,error}=await supabase
      .from("comment_reports")
      .select("id,reporter_id,comment_id,listing_id,reported_user_id,reason,details,comment_body_snapshot,status,created_at")
      .order("created_at",{ascending:false})
      .limit(100);
  ...
  async function refreshComments(listingId:string){
    ...
    const{data,error}=await supabase
      ...
      .select("id,listing_id,user_id,body,status,created_at")
  ...
  function openComments(x:Listing){
    setCommentsOpen(x);
    setComments([]);
    setCommentText("");
    void refreshComments(x.id);
  }

  async function submitComment(e:FormEvent<HTMLFormElement>){
    e.preventDefault();
    if(!supabase||!commentsOpen)return;
    ...
    if(!body)return show("請輸入留言內容");
    if(body.length>500)return show("留言最多 500 字");

    const{error}=await supabase
      .from("listing_comments")
      .insert({
        listing_id:commentsOpen.id,
        user_id:user.id,
        body
      });
  ...
  async function deleteComment(comment:ListingComment){
    if(!supabase||!user||!commentsOpen)return;

    const canDelete=
      comment.user_id===user.id||
      commentsOpen.user_id===user.id||
      isAdmin;

    if(!canDelete)return show("你沒有刪除這則留言的權限");
  ...
  async function reportComment(comment:ListingComment){
    if(!supabase||!user||!commentsOpen)return;
    if(comment.user_id===user.id)return show("不能檢舉自己的留言");

    const details=window.prompt("請簡短說明檢舉原因（可留空）");
    ...
    const{error}=await supabase
      ...
      .insert({
        ...
        comment_body_snapshot:comment.body
      });

    if(error){
      if(error.code==="23505")return show("你已經檢舉過這則留言");
      return show(friendlyError(error.message));
    }
  ...
  async function updateCommentReportStatus(
    reportId:string,
    ...
    show(status==="dismissed"?"已駁回留言檢舉":status==="actioned"?"已完成留言處置":"已標記留言審核");
  }

  async function adminHideComment(commentId:string,reportId:string){
    if(!supabase||!user||!isAdmin)return;
    ...
    if(reportError)return show(friendlyError(reportError.message));
  ...
  return <>
      ...
      <main className="wrap">
        <section className="hero">
          <div className="heroShade">
            ...
            <h1>找到一起冒險的人。</h1>
            <p>祭司、組隊任務、BOSS、公會、長期夥伴。資料會真正綁定會員帳號並由所有玩家共用。</p>
          </div>
        </section>
        ...
        <div className="panel" style={{marginTop:18}}>
          ...
          <div className="muted" style={{marginTop:6}}>
            ...
            祭司媒合、組隊任務與 BOSS 將依遊戲版本與玩家需求逐步開放。
          </div>
        </div>
    ...
    {screen==="favorites"&&<>
      ...
      <main className="wrap">
        <section className="plain">
          <h1>我的收藏</h1>
          <p>把想之後再聯絡的祭司、隊伍、BOSS、公會或夥伴先存起來。</p>
        </section>
        <div className="sectionTitle">
          <h2>已收藏刊登</h2>
        ...
        {!isAdmin
          ...
          :<>
            <div className="sectionTitle">
              ...
              <p>
                {adminReports.filter(r=>r.status==="pending").length+
                  adminCommentReports.filter(r=>r.status==="pending").length} 筆
              </p>
            </div>
            ...
            <div className="sectionTitle" style={{marginTop:26}}>
              <h2>留言檢舉</h2>
              <p>{adminCommentReports.filter(r=>r.status==="pending").length} 筆待處理</p>
            </div>

            <div className="grid">
              {adminCommentReports.length===0&&
                <div className="empty">目前沒有留言檢舉資料。</div>
              }

              {adminCommentReports.map(report=>{
                const statusLabel:Record<string,string>={
                  pending:"待審核",
                  reviewed:"已審核",
                  dismissed:"已駁回",
                ...
                return <article className="card" key={report.id}>
                  <div className="cardTop">
                    <div>
                      <span className="eyebrow">留言檢舉</span>
                      <h3>「{report.comment_body_snapshot}」</h3>
                    </div>
                  ...
                  <div className="actions">
                    {report.status==="pending"&&<>
                      <button
                        className="btn soft"
                        onClick={()=>updateCommentReportStatus(report.id,"reviewed")}
                      >
                        標記已審核
                      </button>
                      <button
                        className="btn soft"
                        onClick={()=>updateCommentReportStatus(report.id,"dismissed")}
                      >
                        駁回
                      </button>
                      <button
                        className="btn danger"
                        onClick={()=>adminHideComment(report.comment_id,report.id)}
                      >
                        隱藏留言
                      </button>
                    </>}
    ...
    {charOpen&&
      <Modal close={()=>setCharOpen(false)}>
        ...
        <form className="form two" onSubmit={addChar}>
          ...
          <label>職業<input name="job" required placeholder="例如：祭司"/></label>
          <label>
            伺服器
    ...
    {commentsOpen&&
      <Modal close={()=>{
        ...
        <div style={{display:"grid",gap:10,marginTop:14,maxHeight:"42vh",overflowY:"auto"}}>
          {commentsLoading
            ?<div className="empty">留言讀取中…</div>
            :comments
              .filter(comment=>!blockedUserIds.includes(comment.user_id))
              .length===0
              ?<div className="empty">目前還沒有留言，來當第一個留言的人吧。</div>
              :comments
                .filter(comment=>!blockedUserIds.includes(comment.user_id))
                .map(comment=>{
                  const ownComment=user?.id===comment.user_id;
                  const listingOwner=commentsOpen.user_id===comment.user_id;
                  ...
                  >
                    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
                      <div>
                        <b style={{fontSize:14}}>
                          {listingOwner?"刊登者":ownComment?"你":"玩家"}
                        </b>
        ...
        {user
          ?<form onSubmit={submitComment} style={{marginTop:14}}>
            <label>
              留言
              <textarea
                rows={3}
            ...
            <div className="modalActions" style={{marginTop:10}}>
              ...
              <button className="btn green" disabled={!commentText.trim()}>
                送出留言
              </button>
            </div>
          </form>
          :<div className="panel" style={{marginTop:14,textAlign:"center"}}>
            <div className="muted">登入後即可留言。</div>
            <button
              type="button"
              className="btn green"
              style={{marginTop:10}}
